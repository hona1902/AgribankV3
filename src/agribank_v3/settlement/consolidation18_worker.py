...
from agribank_v3.settlement.processors import Mau18Processor
...
def _cash_key(record) -> str:
    currency = str(record.currency or "").strip() or "VND"
    account = str(record.account or "").strip()
    return f"{account}_{currency}" if account else f"KoCoTK_{currency}"
...
def _add_summary_sheet(workbook, records, profile: BranchProfile) -> None:
    ...
    sheet = workbook.create_sheet("TongHop_Mau18")
    ...
    cash_keys = tuple(sorted({_cash_key(record) for record in records}))
    ...
    sheet.cell(1, 1, "Báo cáo tổng hợp số liệu quyết toán mẫu 18")
...
def main() -> int:
    if len(sys.argv) != 2:
        print(
            "Usage: python -m agribank_v3.settlement.consolidation18_worker <request.json>",
            file=sys.stderr,
        )
        return 2
    ...
    spec = SETTLEMENT_SPECS["consolidation.18"]
    started = time.perf_counter()
    merge_result = None
    records = []
    try:
        _log(f"[TongHop18] start: {len(source_paths)} source files -> {output_path}")
        step = time.perf_counter()
        merge_result = merge_same_structure_csv_to_csv(source_paths, merged_csv_path)
        _log(
            f"[TongHop18] merge done in {time.perf_counter() - step:.1f}s, "
        ...
        _log(
            f"[TongHop18] read/sort done in {time.perf_counter() - step:.1f}s, "
            f"records={len(records)}"
        )
        step = time.perf_counter()
        workbook = processor.build_workbook(request, records, report_date)
        workbook.active.title = "SoLieu_Mau18"
        ...
        try:
            ...
            workbook.close()
        _log(
            f"[TongHop18] save done in {time.perf_counter() - step:.1f}s; "
