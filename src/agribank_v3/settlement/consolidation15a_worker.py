...
def _add_summary_sheet(workbook, records, profile: BranchProfile) -> None:
    ...
    sheet = workbook.create_sheet("TongHop_Mau15a")
        ...
        sorted(
            {
                ...
                for record in records
                if str(record.interest_account).strip()[:3] in {"394", "488"}
            }
        )
    )
    ...
    def cash_value(branch: str, key: str) -> Decimal:
        return sum(
            (
                ...
                for record in records
                if _branch_from_customer_id(record.customer_id, fallback_branch) == branch
                and f"{record.account}_{str(record.currency or '').strip() or 'VND'}" == key
            ),
            Decimal(0),
        )
    ...
    def accrual_value(branch: str, account: str) -> Decimal:
        return sum(
            (
                ...
                if str(record.interest_account).startswith("488")
    ...
    cash_end = write_table(
        1,
        "Báo cáo tổng hợp số liệu quyết toán mẫu 15a",
        cash_headers,
        cash_rows,
    )
            ...
            [
                ...
                *values,
                sum(
                    (
                        value
                        for account, value in zip(accrual_accounts, values)
                        if not str(account).startswith("488")
                    ),
                    Decimal(0),
                ),
            ]
    ...
    write_table(
        cash_end + 3,
        "Số liệu dự thu đến 31/12 mẫu 15a/QT",
        accrual_headers,
        accrual_rows,
    )
...
def main() -> int:
    if len(sys.argv) != 2:
        print(
            "Usage: python -m agribank_v3.settlement.consolidation15a_worker <request.json>",
            file=sys.stderr,
        )
        return 2
    ...
    spec = SETTLEMENT_SPECS["consolidation.15a"]
    started = time.perf_counter()
    merge_result = None
    records = []
    try:
        _log(f"[TongHop15a] start: {len(source_paths)} source files -> {output_path}")
        step = time.perf_counter()
        merge_result = merge_same_structure_csv_to_csv(source_paths, merged_csv_path)
        _log(
            f"[TongHop15a] merge done in {time.perf_counter() - step:.1f}s, "
        ...
        _log(
            f"[TongHop15a] read/sort done in {time.perf_counter() - step:.1f}s, "
            f"records={len(records)}"
        )
        step = time.perf_counter()
        workbook = processor.build_workbook(request, records, report_date)
        workbook.active.title = "SoLieu_Mau15a"
        ...
        try:
            ...
            workbook.close()
        _log(
            f"[TongHop15a] save done in {time.perf_counter() - step:.1f}s; "
            f"total={time.perf_counter() - started:.1f}s"
        )
        _log(
            "[TongHop15a] completed: "
