...
def main() -> int:
    if len(sys.argv) != 2:
        print(
            "Usage: python -m agribank_v3.settlement.consolidation1314_worker <request.json>",
            file=sys.stderr,
        )
        return 2
    ...
    records = []
    try:
        _log(f"[TongHop{report_code}] start: {len(source_paths)} source files -> {output_path}")
        step = time.perf_counter()
        merge_result = merge_same_structure_csv_to_csv(source_paths, merged_csv_path)
        _log(
            f"[TongHop{report_code}] merge done in {time.perf_counter() - step:.1f}s, "
        ...
        _log(
            f"[TongHop{report_code}] read/sort done in {time.perf_counter() - step:.1f}s, "
        ...
        if spec_key == "consolidation.13":
            ...
            try:
                ...
                workbook.close()
        _log(
            f"[TongHop{report_code}] save done in {time.perf_counter() - step:.1f}s; "
            f"total={time.perf_counter() - started:.1f}s"
        )
        _log(
            f"[TongHop{report_code}] completed: "
